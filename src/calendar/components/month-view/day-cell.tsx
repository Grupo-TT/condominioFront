...
export function DayCell({ cell, events, eventPositions }: IProps) {
  ...
  return (
    <DroppableDayCell cell={cell}>
      <div className={cn("flex h-full flex-col gap-1 border-l border-t py-1.5 lg:py-2 min-h-[114px]", isSunday && "border-l-0")}>
        <span
          className={cn(
            "h-6 px-1 text-xs font-semibold lg:px-2 flex-shrink-0",
        ...
        <div className={cn("flex h-6 gap-1 px-2 lg:flex-1 lg:flex-col lg:gap-1.5 lg:px-1 lg:overflow-y-auto", !currentMonth && "opacity-50")}>
          {[0, 1, 2].map(position => {
