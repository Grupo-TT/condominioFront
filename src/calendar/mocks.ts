...
const createRecursoComun = (nombre: string, tipo: 'ZONA' | 'OBJETO' = 'OBJETO'): RecursoComun => ({
  nombre,
  descripcion: `Recurso: ${nombre}`,
  tipoRecursoComun: tipo,
});
...
const mockGenerator = (numberOfEvents: number): IEvent[] => {
  const result: IEvent[] = [
    {
      ...
      user: USERS_MOCK[0],
      recursoComun: createRecursoComun("My wedding :)"),
    },
  ];
  ...
  const currentEventTitle = EVENTS[Math.floor(Math.random() * EVENTS.length)];
  const currentEvent = {
    id: currentId++,
    ...
    user: randomUser,
    recursoComun: createRecursoComun(currentEventTitle),
  };
  ...
  while (i < numberOfEvents - 1 && attempts < maxAttempts) {
    ...
    result.push({
      ...
      recursoComun: createRecursoComun(eventTitle),
    });
