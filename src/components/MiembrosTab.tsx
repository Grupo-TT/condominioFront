...
import { Search, X, Plus, Users, MoreVertical, Pencil, Trash2, CheckCircle2, XCircle, PawPrint } from 'lucide-react'
...
export function MiembrosTab() {
  ...
  const [isAgregarMiembroSheetOpen, setIsAgregarMiembroSheetOpen] = useState(false)
  ...
  const columns: ColumnDef<MiembroHogar>[] = useMemo(() => [
    ...
    },
    {
      ...
      id: 'telefono',
      header: ({ column }) => <DataGridColumnHeader title="Teléfono" column={column} />,
      cell: ({ row }) => (
        <div className={row.original.telefono ? "text-gray-900" : "text-gray-500 opacity-50"}>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <DropdownMenuItem
                    className={row.original.estado === 'Activo' ? 'text-red-600 focus:text-red-600 focus:bg-red-50' : 'text-green-700 focus:text-green-700 focus:bg-green-50'}
                  ...
                  >
                    ...
                    {row.original.estado === 'Activo' ? 'Deshabilitar' : 'Habilitar'}
                  </DropdownMenuItem>
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                  ...
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction
                      onClick={() => {
                        ...
                        console.log('Cambiar estado de', row.original.nombre)
                      }}
                      className={row.original.estado === 'Activo' ? 'bg-red-600 hover:bg-red-700' : 'text-white hover:opacity-90'}
                      style={row.original.estado === 'Activo' ? undefined : { backgroundColor: '#4C6C5B' }}
                    >
                      Confirmar
                    </AlertDialogAction>
                  </AlertDialogFooter>
              ...
              <DropdownMenuItem
                onClick={() => {
                  setMiembroParaEditar(row.original)
                ...
                {estadoFilter !== 'todos' ? (
                  <span className="flex items-center gap-2.5">
                    <span className={cn(
                      ...
                      estadoFilter === 'Activo' ? 'bg-green-500' : 'bg-gray-500'
                    )}></span>
          ...
          <Popover open={agregarMenuOpen} onOpenChange={setAgregarMenuOpen}>
            ...
            <PopoverContent className="w-auto p-3" align="end">
              <div className="flex gap-3">
                ...
                >
                  <HugeiconsIcon icon={User03Icon} className="w-7 h-7 text-gray-700" />
                  <span className="text-sm font-medium text-gray-900">Miembro</span>
                </button>
                ...
                >
                  ...
                  <span className="text-sm font-medium text-gray-900">Mascota</span>
                </button>
              </div>
            </PopoverContent>
          </Popover>
